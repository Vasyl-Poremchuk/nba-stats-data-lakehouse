...
def run() -> None:
    ...
    for object_metadata in objects_metadata:
        ...
        top_performer_by_points_dyf = season_etl.get_dyf(
        ...
        season_etl.write_to_iceberg(
            df=passed_top_performer_by_points_df,
            catalog=CATALOG,
            db_name=db_name,
            table_name=TopPerformer.BY_POINTS,
        )
        season_etl.write_to_iceberg(
            df=passed_top_performer_by_rebounds_df,
            catalog=CATALOG,
            db_name=db_name,
            table_name=TopPerformer.BY_REBOUNDS,
        )
        season_etl.write_to_iceberg(
            df=passed_top_performer_by_assists_df,
            catalog=CATALOG,
            db_name=db_name,
            table_name=TopPerformer.BY_ASSISTS,
        )
        season_etl.write_to_iceberg(
            df=passed_top_performer_by_win_shares_df,
            catalog=CATALOG,
            db_name=db_name,
            table_name=TopPerformer.BY_WIN_SHARES,
        )
        ...
        season_etl.write_to_iceberg(
            df=failed_top_performer_by_points_df,
            catalog=CATALOG,
            db_name=db_name,
            table_name=f"{TopPerformer.BY_POINTS}_quarantine",
        )
        season_etl.write_to_iceberg(
            df=failed_top_performer_by_rebounds_df,
            catalog=CATALOG,
            db_name=db_name,
            table_name=f"{TopPerformer.BY_REBOUNDS}_quarantine",
        )
        season_etl.write_to_iceberg(
            df=failed_top_performer_by_assists_df,
            catalog=CATALOG,
            db_name=db_name,
            table_name=f"{TopPerformer.BY_ASSISTS}_quarantine",
        )
        season_etl.write_to_iceberg(
            df=failed_top_performer_by_win_shares_df,
            catalog=CATALOG,
            db_name=db_name,
            table_name=f"{TopPerformer.BY_WIN_SHARES}_quarantine",
        )
